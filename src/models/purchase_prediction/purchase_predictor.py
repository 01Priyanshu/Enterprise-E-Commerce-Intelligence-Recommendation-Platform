"""
Purchase Prediction Model using XGBoost
Predicts likelihood of customer making a purchase in next 30 days
"""
import pandas as pd
import numpy as np
import joblib
...
class PurchasePredictor:
    ...
    def __init__(self):
        ...
        self.feature_names = []
        self.model_path = MODELS_DIR / "purchase_prediction"
    ...
    def create_target(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Define active customers (purchased within last 30 days)
        df['will_purchase'] = (df['days_since_last_order'] <= 30).astype(int)
        
        active_count = df['will_purchase'].sum()
        inactive_count = len(df) - active_count
        
        logger.info(f"Target distribution:")
        logger.info(f"  Will purchase (1): {active_count} ({active_count/len(df)*100:.1f}%)")
        logger.info(f"  Won't purchase (0): {inactive_count} ({inactive_count/len(df)*100:.1f}%)")
    ...
    def predict(self, df: pd.DataFrame):
        """Predict purchase probability for customers"""
...
def main():
    """Train and save purchase prediction model"""
    logger.info("="*60)
    logger.info("PURCHASE PREDICTION MODEL TRAINING")
    logger.info("="*60)
    ...
    logger.info("="*60)
    logger.info("PURCHASE PREDICTION MODEL COMPLETE!")
    logger.info("="*60)
